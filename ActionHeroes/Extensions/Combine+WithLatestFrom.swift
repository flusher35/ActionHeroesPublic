...
public extension Publisher {
    ...
    ///  Merges five publishers into a single publisher by combining each value
    ///  from self with the latest value from the second, third, fourth, fifth publisher, if any.
    ///
    ///  - parameter other: A second publisher source.
    ///  - parameter other1: A third publisher source.
    ///  - parameter other2: A fourth publisher source.
    ///  - parameter other3: A fifth publisher source.
    ///  - parameter resultSelector: Function to invoke for each value from the self combined
    ///                              with the latest value from the second, third and fourth source, if any.
    ///
    ///  - returns: A publisher containing the result of combining each value of the self
    ///             with the latest value from the second, third, fourth, fifth publisher, if any, using the
    ///             specified result selector function.
    func withLatestFrom<Other: Publisher, Other1: Publisher, Other2: Publisher, Other3: Publisher, Result>(_ other: Other,
                                                                                                           _ other1: Other1,
                                                                                                           _ other2: Other2,
                                                                                                           _ other3: Other3,
                                                                                                           resultSelector: @escaping (Output, (Other.Output, Other1.Output, Other2.Output, Other3.Output)) -> Result)
    -> Publishers.WithLatestFrom<Self, AnyPublisher<(Other.Output, Other1.Output, Other2.Output, Other3.Output), Self.Failure>, Result>
    where Other.Failure == Failure, Other1.Failure == Failure, Other2.Failure == Failure, Other3.Failure == Failure {
        let combined = other.combineLatest(other1, other2, other3)
            .eraseToAnyPublisher()
        return .init(upstream: self, second: combined, resultSelector: resultSelector)
    }
